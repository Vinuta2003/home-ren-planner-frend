...
function PhaseForm() {
  ...
  useEffect(() => {
    if (selectedRenovationType) {
      axiosInstance.get(`/phase/phases/by-renovation-type/${selectedRenovationType}`)
  ...
  return (
    <div className="min-h-screen bg-blue-50 flex justify-center items-center">
      <form onSubmit={handleSubmit} className="bg-white p-8 rounded-xl shadow-md w-full max-w-xl">
        ...
        <input type="text" name="phaseName" placeholder="Phase Name" value={formData.phaseName} onChange={handleChange}
        ...
        <input type="text" name="vendor" placeholder="Vendor ID" value={formData.vendor} onChange={handleChange}
          className="w-full px-4 py-2 border border-gray-300 rounded mb-3 focus:outline-none focus:ring-2 focus:ring-blue-500" />

        <input type="text" name="project" placeholder="Project ID" value={formData.project} onChange={handleChange}
          className="w-full px-4 py-2 border border-gray-300 rounded mb-3 focus:outline-none focus:ring-2 focus:ring-blue-500" />

        <select value={selectedRenovationType} onChange={(e) => setSelectedRenovationType(e.target.value)}
